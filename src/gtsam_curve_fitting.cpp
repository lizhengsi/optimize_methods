...
class curvfitFactor : public gtsam::NoiseModelFactor1<gtsam::Vector3> {
 ...
 public:
  curvfitFactor(gtsam::Key key, const gtsam::SharedNoiseModel &noise, double x,
                double y)
...
int main() {
  ...
  for (int i = 0; i < 100; ++i) {
    double mx = i / 100.0;
    auto my =
        funct(para, mx) + rng.gaussian(w_sigma * w_sigma);  // 加入了噪声数据
    auto noiseM = gtsam::noiseModel::Isotropic::Sigma(
        1, w_sigma);  // 噪声的维度需要与观测值维度保持一致
